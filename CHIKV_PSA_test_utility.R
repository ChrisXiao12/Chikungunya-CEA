...
Phi_draws <- rbeta(1000,171.0528,1.902509)
Phi_VIM_draws <- rbeta(1000,516.3816,11.61595)
...
run_SVEIRD4 <- function(params) {
  ...
  names <- c("S", "E", "V", "I", "R", "C", "D", "N", "Check",
             "SV", "SE", "SD", "VE", "VD", "EI", "ED", "IR", "IC", "ID", "RD", "CR", "CD")
  ...
  for(i in 2:length(trace_v$S)){
    trace_v$SV[i] <- trace_v$S[i-1] * ppsi
    trace_v$SE[i] <- trace_v$S[i-1] * pbeta * trace_v$I[i-1] / trace_v$N[i-1]
    trace_v$SD[i] <- trace_v$S[i-1] * pmu
    trace_v$VE[i] <- trace_v$V[i-1] * (1 - pphi) * pbeta * trace_v$I[i-1] / trace_v$N[i-1]
    ...
    trace_v$S[i] <- trace_v$S[i-1] - trace_v$SV[i] - trace_v$SE[i] - trace_v$SD[i]
    trace_v$E[i] <- trace_v$E[i-1] - trace_v$EI[i] - trace_v$ED[i] + trace_v$VE[i] + trace_v$SE[i]
    trace_v$V[i] <- trace_v$V[i-1] - trace_v$VE[i] - trace_v$VD[i] + trace_v$SV[i]
...
clusterExport(cl, varlist = c("parametersdf", "run_SVEIRD4"))
...
clusterExport(cl, varlist = c("Vimkunyaparametersdf", "run_SVEIRD4"))
...
Vimresults_list <- parLapply(cl, 1:1000, function(i) {
  ...
  run_SVEIRD4(Vimkunyaparametersdf[i, ])
})
...
#----
#NMB calculations
nmb1kv <- resultsdf$v_eff_d * 1000 - resultsdf$v_cost_d
nmb1knv <- resultsdf$nv_eff_d * 1000 - resultsdf$nv_cost_d
nmb1kVIM <- Vimresultsdf$v_eff_d * 1000 - Vimresultsdf$v_cost_d

nmb2500v <- resultsdf$v_eff_d * 2500 - resultsdf$v_cost_d
nmb2500nv <- resultsdf$nv_eff_d * 2500 - resultsdf$nv_cost_d
nmb2500VIM <- Vimresultsdf$v_eff_d * 2500 - Vimresultsdf$v_cost_d

nmb5kv <- resultsdf$v_eff_d * 5000 - resultsdf$v_cost_d
nmb5knv <- resultsdf$nv_eff_d * 5000 - resultsdf$nv_cost_d
nmb5kVIM <- Vimresultsdf$v_eff_d * 5000 - Vimresultsdf$v_cost_d

nmb6808v <- resultsdf$v_eff_d * 6808 - resultsdf$v_cost_d
nmb6808nv <- resultsdf$nv_eff_d * 6808 - resultsdf$nv_cost_d
nmb6808VIM <- Vimresultsdf$v_eff_d * 6808 - Vimresultsdf$v_cost_d

nmb10kv <- resultsdf$v_eff_d * 10000 - resultsdf$v_cost_d
nmb10knv <- resultsdf$nv_eff_d * 10000 - resultsdf$nv_cost_d
nmb10kVIM <- Vimresultsdf$v_eff_d * 10000 - Vimresultsdf$v_cost_d

nmb15kv <- resultsdf$v_eff_d * 15000 - resultsdf$v_cost_d
nmb15knv <- resultsdf$nv_eff_d * 15000 - resultsdf$nv_cost_d
nmb15kVIM <- Vimresultsdf$v_eff_d * 15000 - Vimresultsdf$v_cost_d

nmb25kv <- resultsdf$v_eff_d * 25000 - resultsdf$v_cost_d
nmb25knv <- resultsdf$nv_eff_d * 25000 - resultsdf$nv_cost_d
nmb25kVIM <- Vimresultsdf$v_eff_d * 25000 - Vimresultsdf$v_cost_d

nmb50kv <- resultsdf$v_eff_d * 50000 - resultsdf$v_cost_d
nmb50knv <- resultsdf$nv_eff_d * 50000 - resultsdf$nv_cost_d
nmb50kVIM <- Vimresultsdf$v_eff_d * 50000 - Vimresultsdf$v_cost_d

nmb100kv <- resultsdf$v_eff_d * 100000 - resultsdf$v_cost_d
nmb100knv <- resultsdf$nv_eff_d * 100000 - resultsdf$nv_cost_d
nmb100kVIM <- Vimresultsdf$v_eff_d * 100000 - Vimresultsdf$v_cost_d


nmb150kv <- resultsdf$v_eff_d * 150000 - resultsdf$v_cost_d
