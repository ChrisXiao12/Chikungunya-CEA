...
##*****************************************************************************************************
## UTILITIES FUNCTIONS (Utilities_Functions.R)
##***************************************************************************************************** 
...
params_adj_function <- function(
  ...
  val[names(cpi_adjust)] <- as.numeric(val[names(cpi_adjust)]) * (1 + as.numeric(cpi_adjust[names(cpi_adjust)]))
  ## output
  return(val)  
}
...
owsa_tornado_plot_func <- function(
  ...
  x_exp_max = 0.01,
  ##
  xlab = "ICER, 2025 US$ per DALY averted",
...
run_single <- function(
  ...
  ## fx factor
  fx_factor <- (1 / as.numeric(params[["brl_usd_fx_2025"]]))
...
psa_plot_func <- function(
  ...
  x_lab = "Willingness-to-pay (WTP) threshold, 2025 US$'000",
...
behavior_table_func <- function(
  ...
  ft <- flextable(sensi_table_ft) %>%
    ...
    bold(part = "header") %>%
    align(
      j = c("beta_S", "xi", "beta_SV", "total_EtoI", "nmb", "inmb", "icer"),
...
phi_pi_omicron_heatmap_func <- function(
  ...
  scale_lab = "INMB (2025 US$ million): ",
  midpoint_val = 2.5,
  title = NULL,
...
##*****************************************************************************************************
## BASE CASE ANALYSIS (BaseCaseAnalysis.R)
##*****************************************************************************************************
...
digits_vals <- list(Costs = 0, QALYs = 0, NMB = 0, Inc_Costs = 0, Inc_QALYs = 0, INMB = 2, ICER = 0)
...
if(isTRUE(create_base_cea_table_S1)){
  cea_table_func(
    ...
    QALYs_lab = "QALYs",
    NMB_lab = "NMB (R$ M)",
    Inc_Costs_lab = "Costs (R$)",
    Inc_QALYs_lab = "QALYs",
    INMB_lab = "INMB (R$ M)",
    ICER_lab = "ICER (R$/QALY)",
    dominance_status_lab = "Dominance Status",
...
##*****************************************************************************************************
## ONE WAY SENSITIVITY ANALYSIS (OneWaySensitivityAnalysis.R)
##***************************************************************************************************** 
...
if(isTRUE(create_tornado_plots)){
  ...
  owsa_tornado_plot_func(
    ...
    x_exp_max = 0.03,
    ##
    ylab = "INMB, 2025 million US$ per 1,000 population",
...
##*****************************************************************************************************
## ADDITIONAL SENSITIVITY ANALYSES (AdditionalSensitivityAnalyses.R)
##*****************************************************************************************************   
...
if(isTRUE(use_saved_sensi_time_horizon_cea_data) & file.exists(sensi_time_horizon_cea_data_file)){
  ...
  sensi_time_horizon_cea_data <- sensi_func(
    ...
    eta_vals = NULL,
    omicron_vals = NULL,
    time_horizon_years = time_horizon_set,
    wtp = NULL,
    scenarios = scenarios, 
    ref_scenario = scenarios[1],
    params = params_value, 
...
##*****************************************************************************************************
## PROBABILISTIC SENSITIVITY ANALYSIS (PSA) (ProbabilisticSensitivityAnalyses.R)
##***************************************************************************************************** 
...
if(isTRUE(create_psa_plots)){
  ...
  psa_plot_func(
    ...
    x_lab = "WTP threshold, 2025 US$'000",
    y_lab = "Probability of being cost-effective",
    ##
    color_lab = NULL,
    fill_lab = NULL,
...
digits_vals <- list(Costs = 0, QALYs = 0, NMB = 0, INMB = 2)
