...
inner_ceac_data_func <- function(
  ...
  val <- data[[paste0("cea_out_", currency_val)]] %>%
    ...
    dplyr::summarise(
      n = n(),
      ##
      SOC_k = sum(best_strategy == "SOC"),
      IXCHIQ_k = sum(best_strategy == "IXCHIQ"),
      VIMKUNYA_k = sum(best_strategy == "VIMKUNYA"),
      ##
      SOC_prob = SOC_k / n,
...
inner_boot_mean_ci_func <- function(
  ...
  data = psa_data,
  nbootsamps = 1000,
  wtp_val = params_value[["GDP_Per_Capita"]],
  scenarios = scenarios
) {
  ...
  summary_boot <- boot_results %>%
    ...
    ungroup() %>%
    arrange(perspective, alternative, reference)

  colnames(summary_boot) <- c(
    "perspective", "alternative", 
    "reference", cols_to_summarise
  )

  out_boot <- summary_boot %>%
    mutate(
      measure = rep(c("mean", "se", "lower", "upper"), nrow(summary_boot)/4)
    ) %>%
    pivot_wider(
      id_cols = c(perspective, alternative, reference), 
      names_from = measure, 
      values_from = all_of(cols_to_summarise),
...
mult_cea_table_func <- function(
  ...
  ## prepare table data: select alternatives and needed columns
  table_data <- psa_boot_data  %>%
    ...
    dplyr::select(
      perspective, alternative, reference,
      costs_mean, costs_lower, costs_upper,
      qalys_mean, qalys_lower, qalys_upper,
      nmb_mean, nmb_lower, nmb_upper,
      inmb_mean, inmb_lower, inmb_upper
    ) %>%
    ## format numeric columns as "estimate (lower, upper)"
    mutate(
      Costs = fmt_ci_journal(
        mean = costs_mean, 
        lower = costs_lower, 
...
sensi_func <- function(
  ...
  iter_combo <- expand.grid(
    ...
    time_horizon = as.numeric(time_horizons),
  ...
  cea_vals_list <- lapply(
    ...
    FUN = function(i) {
      ...
      time_horizon_val <- iter_combo$time_horizon[i]
...
behavior_table_func <- function(
  ...
  sensi_inc <- data$trace %>%
    ...
    dplyr::select(cycle, scenario, beta_S, xi, beta_SV, EtoI) %>%
    group_by(scenario, xi) %>%
  ...
  sensi_cea <- data[[paste0("cea_out_", currency)]] %>%
    ...
    dplyr::select(
      scenario, beta_S, xi, beta_SV, costs, qalys, 
  ...
  sensi_table <- merge(
    x = sensi_inc, 
    y = sensi_cea, 
    by = c("scenario", "beta_S", "xi", "beta_SV")
  )
  ...
  sensi_table_ft <- sensi_table %>%
    ...
    dplyr::select(scenario, beta_S, xi, beta_SV, total_EtoI, nmb, icer)
  ...
  ft <- flextable(sensi_table_ft) %>%
    ...
    bold(part = "header") %>%
    align(j = c("beta_S", "xi", "beta_SV", "total_EtoI", "nmb", "icer"),
