...
##----------------------------------------------------------------------------------------
## PSA draws
R0_draws <- rgamma(n_psa_draws, shape = 54.8258, scale = 0.0620146)
Gamma_draws <- rgamma(n_psa_draws, shape = 115.7945, scale = 0.007111992)
Beta_draws <- R0_draws * Gamma_draws
Lambda_draws <- rgamma(n_psa_draws, shape = 0.7314914, scale = 1.913898)
Delta_draws <- rbeta(n_psa_draws, 61.40313, 61341.73)
Phi_draws <- rbeta(n_psa_draws, 260.2326, 8.602732)
Phi_R_draws <- rbeta(n_psa_draws, 1935.908, 328.3118)
Psi_draws <- rgamma(n_psa_draws, shape = 61.4656, scale = 0.0001708273)
Mu_draws <- rgamma(n_psa_draws, shape = 61.4656, scale = 0.0000001130876)
Kappa_draws <- rbeta(n_psa_draws, 2.392294, 122.3488)
Omega_draws <- rbeta(n_psa_draws, 95.95506, 79.02587)

## Theta???

## Transform draws to probabilities
...
discount_draws <- rep(common_base_case$discount, n_psa_draws)

pop_draws <- rep(n_cohort, n_psa_draws)
fracinfected_draws <- rbeta(n_psa_draws, 61.03465, 30532.77)
fracsusceptible_draws <- 1 - fracinfected_draws
cyclelength_draws <- rep(cycle_length, n_psa_draws)
R_adverse_L <- rbeta(n_psa_draws, 60.27875, 3112.287)
R_adverse_R <- rbeta(n_psa_draws, 10.81465, 405.1335)
U_S_draws <- rbeta(n_psa_draws, 28691.07, 6128.19)
U_R_draws <- U_S_draws

Theta_L_draws <- rbeta(n_psa_draws, 61.44024, 151269.2)
Theta_R_draws <- rbeta(n_psa_draws, 61.10256, 11144.21)

U_E_draws_raw <- rbeta(n_psa_draws, 23.90391, 10.24453)
U_I_draws_raw <- rbeta(n_psa_draws, 138.2644, 70.59423)
...
U_C_draws <- rbeta(n_psa_draws, 5.088638, 5.212248)
U_D_draws <- rep(0, n_psa_draws)
C_S_draws <- rep(0, n_psa_draws)
Vax_Dose_draws_L <- rgamma(n_psa_draws, shape = 104.7666, scale = 0.2243081)
Vax_Dose_draws_R <- rgamma(n_psa_draws, shape = 104.7666, scale = 0.2243081)
Admin_draws <- rgamma(n_psa_draws, shape = 61.4656, scale = 0.04522855)
Wastage_draws <- rpert(n_psa_draws, min = 0.05, mode = 0.1, max = 0.15)
C_V_draws <- Vax_Dose_draws_L / (1 - Wastage_draws) + Admin_draws #adds vaccine wastage and admin cost
C_VR_draws <- Vax_Dose_draws_R / (1 - Wastage_draws) + Admin_draws
D_hosp_draws <- rgamma(n_psa_draws, shape = 61.57906, scale = 0.1761963)
Cost_hosp_draws <- rgamma(n_psa_draws, shape = 61.46702, scale = 14.12546)
P_hosp_draws <- rbeta(n_psa_draws, 60.43122,3291.949)
Infectious_direct <- rgamma(n_psa_draws, shape = 61.40944, scale = 0.1424862)
Chronic_direct <- rgamma(n_psa_draws, shape = 61.465627, scale = 1.286606)
Infectious_absenteeism_days <- rgamma(n_psa_draws,shape = 1.356759, scale = 4.599195)
Infectious_absenteeism_freq <- rbeta(n_psa_draws, 26.27682, 3.247697)
Infectious_caregiving_days <- rgamma(n_psa_draws, shape = 11.8879, scale = 0.4054543)
Infectious_caregiving_freq <- rbeta(n_psa_draws, 708.5952, 3228.045)
Chronic_absenteeism_days <- rgamma(n_psa_draws, shape = 96.1739, scale = 0.1493129)
Chronic_absenteeism_freq <- rbeta(n_psa_draws, 56.03872, 8.373601)
Chronic_caregiving_days <- rgamma(n_psa_draws, shape = 95.20305, scale = 0.1197441)
Chronic_caregiving_freq <- rbeta(n_psa_draws, 197.3991, 383.1864)

## Productivity (absenteeism) costs
Absenteeism_cost <- rgamma(n_psa_draws, shape = 61.4656, scale = 0.3143222)

## Caregiving costs
Caregiving_cost <- rgamma(n_psa_draws, shape = 61.4656, scale = 0.3266868)

## Indirect costs (infectious and chronic stages)
TC_indirect_infectious <- Absenteeism_cost * (Infectious_absenteeism_days * Infectious_absenteeism_freq) + 
  Caregiving_cost * (Infectious_caregiving_days * Infectious_caregiving_freq)
TC_indirect_chronic <- Absenteeism_cost * (Chronic_absenteeism_days * Chronic_absenteeism_freq) + 
  Caregiving_cost * (Chronic_caregiving_days * Chronic_caregiving_freq)

C_E_draws <- rep(0, n_psa_draws)
C_I_draws <- ((Cost_hosp_draws / D_hosp_draws) * 7 * P_hosp_draws) + (Infectious_direct) + TC_indirect_infectious
C_R_draws <- rep(0, n_psa_draws)
C_C_draws <- (Chronic_direct / 270) * 7 + TC_indirect_chronic * cycle_length
C_D_draws <- rep(0, n_psa_draws)
...
run_SVEIRD5 <- function(params, return_trace = FALSE) {
  ...
  ## Subtract the disutilities from adverse events related to vaccination
...
wtp_values <- seq(n_psa_draws,150000, by = 1000)
...
##----------------------------------------------------------------------------------------
## Assumed WTP threshold (2024 USD)
wtp <- 7000
...
df_Live <- data.frame(
  ...
  base_case = c(
    ...
    0.018995353, 0.521, 0.03, n_psa_draws, 0.9980005, 0.0019995, 0.019230769,
    0.824, 0.00024, 0.700, 0.662, 0.824, 0.494, 0,
    0, 28.89, 0, 143.61, 0, 8.19, 0, 0.000406
  ),
  LB = c(
    0.813626024, 0.441964854, 0.503414696, 0.00075, 0.975, 0.007844073, 5.21324e-06,
    0.009543213, 0.445, 0.03, n_psa_draws, 0.9985, 0.0015, 0,
    0.820, 0.00018, 0.525, 0.593, 0.820, 0.176, 0,
    0, 22.09, 0, 19.48, 0, 6.14, 0, 0.000305
  ),
  UB = c(
    0.985004423, 0.999088118, 0.632120559, 0.00125, 0.998, 0.013039243, 8.68871e-06,
    0.055910442, 0.597, 0.03, n_psa_draws, 0.9975, 0.0025, 0,
    0.828, 0.00030, 0.828, 0.721, 0.828, 0.759, 0,
    0, 36.42, 0, 567.33, 0, 10.23, 0, 0.000508
  )
)
...
## Loop for each variable calling get_inmb and storing the result, 
## and then adding it to the dataframe
...
## Live-Attenuated Vacine vs No vaccination
BaseL <- INMB_livevsnovax
sorted_L <- DSA_Live_wide %>%
  arrange(desc(range)) %>%
  pull(Parameter) 

plot_dataL <- DSA_Live_wide %>% 
  pivot_longer(
...
## Recombinant Vacine vs No vaccination
BaseR <- INMB_recombovsnovax
...
pL <- ggplot(plot_dataL) +
  ...
  annotate("text", x = BaseL + 0.2, y = 0.3, 
            label = paste0("Live-attenuated Vaccine vs No Vaccination\n(INMB = 2024 US$", formatC(BaseL, format = "f", big.mark = ",", digits = 0), ")"), 
            size = 3, col = "black", hjust = -0.025) +
  geom_vline(xintercept = BaseR, linetype = "dashed", color = "#747678FF") +
  annotate("text", x = BaseR - 0.2, y = 0.3, 
            label = paste0("Recombinant Vaccine vs No Vaccination\n(INMB = 2024 US$", formatC(BaseR, format = "f", big.mark = ",", digits = 0), ")"), 
            size = 3, col = "#747678FF", hjust = 1+0.025) +
  scale_color_manual(values = c("Lower Bound" = "#3182bd", "Upper Bound" = "#6baed6")) +
  scale_x_continuous(
    limits = c(0, 1.3e6),
    breaks = seq(0, 1.3e6, by = 1e5),
    labels = function(x) sprintf("%.1f", x / 1e6), 
    expand = expansion(mult = c(0.005, 0.005))
  ) +
  scale_y_discrete(labels = param_labels, expand = expansion(mult = c(0.1, 0.05))) +
  labs(
    x = "INMB (Millions 2024 USD)",
    y = "Parameter"
  ) +
  theme_minimal() +
  theme(
...
ggsave(
  filename = "INMB_LiveAttenuated_Tornado.png",
  plot = pL,
  path = figures_dir,
  width = 8, height = 6,
...
## Save the plot as a PNG file
ggsave(
  filename = "INMB_Recombinant_Tornado.png",
  plot = pR,
  path = figures_dir,
  width = 8, height = 6,
  units = c("in", "cm", "mm", "px")[1], 
...
##----------------------------------------------------------------------------------------
## Bootstrapping
## Adapted from Data Science in R: A Gentle Introduction, James G. Scott, August 2021, 
...
## Create nbootsamps bootstrap samples from the PSA results
bootLCosts <- do(nbootsamps) * mean(~v_cost_d, data = mosaic::resample(resultsdf))
bootLEff <- do(nbootsamps) * mean(~v_eff_d, data = mosaic::resample(resultsdf))
bootNVCosts <- do(nbootsamps) * mean(~nv_cost_d, data = mosaic::resample(resultsdf))
bootNVEff <- do(nbootsamps) * mean(~nv_eff_d, data = mosaic::resample(resultsdf))
bootRCosts <- do(nbootsamps) * mean(~v_cost_d, data = mosaic::resample(Recomboresultsdf))
bootREff <- do(nbootsamps) * mean(~v_eff_d, data = mosaic::resample(Recomboresultsdf))
bootNMBNV <- do(nbootsamps) * mean(~NoVax, data = mosaic::resample(nmb7000))
bootNMBR <- do(nbootsamps) * mean(~Recombo, data = mosaic::resample(nmb7000))
bootNMBL <- do(nbootsamps) * mean(~Live, data = mosaic::resample(nmb7000))
bootINMB_LNV <- do(nbootsamps) * mean(~INMB_LNV, data = mosaic::resample(nmb7000))
bootINMB_RNV <- do(nbootsamps) * mean(~INMB_RNV, data = mosaic::resample(nmb7000))
bootINMBLR <- do(nbootsamps) * mean(~INMBLR, data = mosaic::resample(nmb7000))

## Compute confidence intervals of the nbootsamps bootstrap samples
...
## WTP thresholds
## https://pmc.ncbi.nlm.nih.gov/articles/PMC9017085/pdf/41283_2022_Article_94.pdf
## https://www.oecd.org/content/dam/oecd/en/publications/reports/2025/10/mortality-risk-valuation-in-policy-assessment_733b9d66/76ca89a2-en.pdf
vsl_brsl <- data.frame(
  gamma = c(0.1, 1, 0.5),
  r = c(0.05, 0.05, 0.05),
  vsl = c(5.0, 0.5, 1.0) * 1e6
)
...
## time horizon
time_horizon <- c(1, seq(from = 5, to = 20, by = 5))
